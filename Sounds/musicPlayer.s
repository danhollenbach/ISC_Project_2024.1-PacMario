...
P_CONT: 
	la t4, BATIDA
	lw t4, (t4)
	lw t0, 8(t4)		#t0 = note_conter
	lw t1, 4(t4)		#t1 = music num

	bne t0,t1, P_NOTE	#If t0 != t1 play note
	
	li t2, 0		#t2 = 0
	sw t2,8(t4)		#store 0 in note conter
...
P_NOTE:	
	la t4, BATIDA
	lw t4, (t4)
	lw t0, 8(t4)		#t0 = Note conter
	li t1, 8		#t1 = 8
	mul t1, t1, t0		#t1 = t0 * 8
	lw t2, 0(t4)		#t2 = notes adress
	add t1, t2, t1		#Adress of the right note
	
	lw a2, 20(t4)		#Read the instrument number
	lw a3, 24(t4)		#Read the instrument number
	lw a0, 0(t1)		#Read the value of the note
	lw a1, 4(t1)		#Read the duration of the note

	sw a1, 16(t4)		#Store the value of the last duration
	...
	sw t3, 12(t4)		#Store the current time in last played
	
	addi t0,t0,1		#Add 1 to conter
	sw t0,8(t4)		#Store the conter in conter		
	ret
